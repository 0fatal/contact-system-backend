...
type IdentifyApply struct {
	...
	Appendix      string        `gorm:"type:varchar(2048);default:''" json:"-"`
	Appendixs     []string      `gorm:"-" json:"Appendix"`                  // 附件
...
func GetIdentifyApplyList() (list []*IdentifyApply, err error) {
	...
	for i, _ := range list {
		list[i].Appendixs = strings.Split(list[i].Appendix, ", ")
	}
	return
}
...
func (ia *IdentifyApply) GetIdentifyDetail() (err error) {
	...
	ia.Appendixs = strings.Split(ia.Appendix, ", ")
	return
}
...
func (ia *IdentifyApply) Check(approve bool, checker *User) (err error) {
	...
	err = db.Model(IdentifyApply{}).Where("id = ?", ia.ID).Update("apply_status", ia.ApplyStatus).Update("checker_id", checker.ID).Error
	return
}
